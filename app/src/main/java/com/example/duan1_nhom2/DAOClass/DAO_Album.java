...
public class DAO_Album {
    ...
    public static void updateAlbumWithImage(Uri uri, Context context, final DialogThemVaCapNhatAlbum dialog, final Albums albums, final TextView textView, ImageView imageView){
        ...
        final StorageReference myStorageRef = FirebaseStorage.getInstance().getReference("AnhAlbums").child(albums.getMaAlbum() + AdditionalFunctions.getFileExtension(context, uri));
    ...
    public static void uploadAlbumWithImage(Uri uri, Context context, final DialogThemVaCapNhatAlbum dialog, final Albums albums, final TextView textView, ImageView imageView){
        final DatabaseReference myDatabaseRef = FirebaseDatabase.getInstance().getReference("Albums");
        final String maAlbum = myDatabaseRef.push().getKey();
        final StorageReference myStorageRef = FirebaseStorage.getInstance().getReference("AnhAlbums").child(maAlbum + AdditionalFunctions.getFileExtension(context, uri));
        final UploadTask uploadTask = myStorageRef.putFile(uri);
        uploadTask.continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
            @Override
            public Task<Uri> then(@NonNull Task<UploadTask.TaskSnapshot> task) throws Exception {
